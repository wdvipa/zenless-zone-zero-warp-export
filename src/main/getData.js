...
const detectGameLocale = async (userPath) => {
  ...
  const arr = ['/miHoYo/绝区零/', '/Cognosphere/ZenlessZoneZero/']
  arr.forEach(str => {
    try {
  ...
  try {
    ...
    const promises = logPaths.map(async logpath => {
      ...
      if (gamePathMch) {
        ...
        const urlMch = cacheText.match(/https[^?]+?\?[^?]+?&auth_appid=webview_gacha&.+?authkey=.+?&game_biz=nap_.+?&plat_type=pc/g)
        if (urlMch) {
...
const getQuerystring = (url) => {
  ...
  if (host.includes('webstatic-sea') || host.includes('hkrpg-api-os') || host.includes('api-os-takumi') || host.includes('hoyoverse.com')) {
    apiDomain = 'https://public-operation-os-nap.mihoyo.com'
  } else {
